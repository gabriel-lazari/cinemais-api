...
describe('UserController', () => {
  ...
  describe('list()', () => {
    it('deve retornar a lista de usuários', async () => {
      const users: UserEntity[] = [
        { id: 1, uuid: 'uuid-1', name: 'Usuário 1', is_active: true, created_at: new Date(), updated_at: new Date() },
        { id: 2, uuid: 'uuid-2', name: 'Usuário 2', is_active: true, created_at: new Date(), updated_at: new Date() }
      ];
  ...
  describe('findById()', () => {
    ...
    it('deve propagar NotFoundException lançado pelo service', async () => {
      service.findById.mockRejectedValue(new NotFoundException('Usuário com id 999 não encontrada'));
